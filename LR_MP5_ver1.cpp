    ...
    if (!input.empty()) {
        ...
        catch (int i)
        {
            ...
            case (1):
                ...
                break;
            case (2):
                cerr << ("Error (1)! Write a positive number!") << endl;
                return -1;
                break;
            }
        }
...
istream& operator>>(istream& in, ProfitDays& PD)
{
    ...
    */
        ...
        if (info.size() < 2) throw runtime_error("Wrong input!");
        else set_all_information(info, PD);
...
void set_file_input(ifstream& in, vector<ProfitDays>& PD_vector)
{
    ...
    {
        cout << e.what() << endl;
        cout << "Skipping the wrong line..." << endl;
    }
    catch (invalid_argument& e)
    {
        cout << e.what() << endl;
